...
def send_single_request(proxy, success_counter, total_counter, lock, client):
    proxy_dict = {
        "http": f"http://{proxy}",
        "https": f"http://{proxy}",
    }
    try:
        response = client.post("https://piratewins.io/?pirate=2458836", headers=headers, proxies=proxy_dict, timeout=10)
        with lock:
            total_counter[0] += 1
